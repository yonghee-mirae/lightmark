...
- **Zoom (완료, macOS 전환 세션에서 구현)**: 계획대로 `--lm-zoom` CSS 변수를 버튼으로 증가/감소, config.json에 쓰지 않는 세션 전용 상태(TOC Toggle과 동일 이유). `lm-toolbar.ts`가 `ZOOM_MIN`(50)/`ZOOM_MAX`(200)/`ZOOM_STEP`(10)/`ZOOM_RESET`(100) 상수를 export해서 `main.ts`가 같은 숫자로 클램프 — 단일 소스. 버튼은 `-`/`0`/`+`가 아니라 `−`(U+2212 MINUS SIGN)/`100%`(현재 배율 표시, 클릭 시 리셋)/`+` — 하이픈/숫자 0이 폭 대비 시각적 무게가 달라 보인다는 사용자 리포트로 교체(디자인 프리뷰 3개 제시 후 선택받음). 상태바의 zoom 표시(`lm-statusbar`, 하드코딩이던 "100%")도 `setZoom()`으로 실제 값 반영. 값 변경은 `applyTheme({ ...currentConfig, zoom })`로 CSS 변수만 재적용(문서 재렌더 없음 — Apply 버튼의 config 전체 재적용보다 가벼움). 상세: `HANDOFF.md`의 "M7 구현 상세 → Zoom" 참고.
...
**검증(전체, 완료)**: TOC Toggle/Zoom/About 세 버튼 모두 구현 완료. Zoom은 프론트 `lint`/`typecheck`/`build`/`test` 통과, 디자인(3버튼 배치, `−`/`100%`/`+` 문자 선택)은 사용자가 단계적으로 직접 확인하며 진행했지만 최종 배선된 클릭 동작 자체에 대한 사용자의 명시적 "정상 동작" 재확인 문구는 아직 없음(macOS 전환 세션 막바지에 멀티 윈도우 조사로 넘어감). TOC Toggle/Zoom 상태가 페이지/앱을 새로 열면 초기화되는 것(세션 전용, 저장 안 됨)은 의도된 동작.
