...
`open_config_file()`/`reset_config()`는 사용자 요청으로 M7에서 제거됨(버튼/IPC 커맨드/플랫폼 어댑터 전부). `reload_config()`가 그 대신 자기 치유(self-heal) 동작을 갖게 됨 — config.json이 없거나 깨져 있으면 읽기만 하는 게 아니라 기본값을 그 자리에 다시 써준다(기존 파일이 있었으면 `config.json.bak`으로 백업). 그냥 읽기(`read_config()`)는 여전히 부작용 없음.
